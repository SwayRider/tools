#!/usr/bin/env python3
"""Runs `go test ./...` for every Go module in go.work, plus `flutter test`
for swayriderapp.
...
DEFAULT_CMD = ["go", "test", "./..."]
MODULE_CMD = {
    "swayriderapp": ["flutter", "test"],
}


def test_cmd(module, extra):
...
def parse_args():
    ...
    parser.add_argument(
        "--modules",
        help="comma-separated subset to test (default: all -- "
        + ",".join(MODULES) + ")",
    )
    parser.add_argument(
        "--fail-fast",
    ...
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="print the resolved `go test` command for each module without "
        "running it",
    )
    parser.add_argument(
        "go_test_args",
        nargs=argparse.REMAINDER,
        help="extra arguments passed through to the test command (e.g. "
        "-- -run Foo -v for `go test`; ignored/module-specific for "
        "swayriderapp's `flutter test`)",
    )
...
def main():
    ...
    if failed:
        ...
        return 1
    print("dry run completed" if args.dry_run else "all tests passed")
    return 0
